...
export const GET_ALL_SEEKERS = gql`
  query GetAllSeekers {
    getAllSeekers {
      id
      email
...
const JobSeekers = () => {
  const { loading, error, data } = useQuery(GET_ALL_SEEKERS);
  ...
  const filteredSeekers = data?.getAllSeekers.filter((user) => {
    const fullName = `${user.firstName} ${user.lastName}`.toLowerCase();
    return fullName.includes(searchQuery.toLowerCase());
  });

  if (loading) return <p className="text-center text-gray-500">Loading seekers...</p>;
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-4xl font-bold text-center text-gray-800 mb-8">All Job Seekers</h1>
        ...
        {/* No seekers found */}
        {filteredSeekers?.length === 0 && searchQuery && (
          <div className="text-center text-gray-500 mt-4">
            <p>No seekers found matching "{searchQuery}".</p>
          </div>
        )}
        ...
        {filteredSeekers?.length === 0 && !searchQuery && (
          <div className="flex flex-col items-center text-center mt-10">
            <img
              ...
              className="w-40 h-40"
            />
            <p className="text-xl font-semibold text-gray-600">No Seekers Found</p>
            <p className="text-sm text-gray-500">No users with the role of "seeker" are available.</p>
          </div>
        )}
      </div>
    </div>
